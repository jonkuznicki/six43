...
export default function DesktopLineupEditor({ params }: { params: { id: string } }) {
  ...
  const [showTip, setShowTip]           = useState(() =>
    typeof window !== 'undefined' ? localStorage.getItem('six43_lineup_tip_seen') !== '1' : true
  )
  ...
  async function loadData() {
    ...
    if (noSlots && gameData?.season_id) {
      ...
      if (prevWithSlots) setHasPriorLineup(true)
      setShowBattingOrderModal(true)
      setLoading(false)
      return
    }

    if (!noSlots) setHasPriorLineup(true)
    setSlots(slotData ?? [])
    setLoading(false)
  }

  async function handleBattingOrderChoice(choice: 'last' | 'roster' | 'fresh') {
    ...
    if (choice === 'last' && prevGameForBatting) {
      ...
      if (freshPlayers?.length) {
        await supabase.from('lineup_slots').insert(
          freshPlayers.map((p: any, i: number) => ({
            game_id: params.id, player_id: p.id,
            batting_order: i + 1, availability: 'available',
            inning_positions: [...BLANK],
          }))
    ...
    <div style={{
      ...
      {tipVisible && viewMode === 'grid' && (
        <div style={{
          ...
          <button
            onClick={() => { setShowTip(false); localStorage.setItem('six43_lineup_tip_seen', '1') }}
      ...
      <div style={{ flex: 1, display: viewMode === 'grid' ? 'flex' : 'none', overflow: 'hidden' }}>
        ...
        <div style={{
          ...
          <div style={{ marginTop: 10, fontSize: 10, color: `rgba(var(--fg-rgb),0.38)`, lineHeight: 1.4 }}>
            Tip: press <code style={{ fontSize: 12, background: 'var(--bg)', padding: '2px 6px', borderRadius: 3,
              border: '0.5px solid var(--border-md)', color: `rgba(var(--fg-rgb),0.75)`,
              fontFamily: 'ui-monospace, monospace', fontWeight: 700 }}>m</code> to assign Center Field
          </div>
