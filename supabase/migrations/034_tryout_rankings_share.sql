-- Migration 034: Rankings share tokens for tryout seasons
-- Allows generating a public read-only link for the rankings view.

alter table tryout_seasons
  add column if not exists rankings_share_token uuid unique default null;

create index if not exists idx_tryout_seasons_share_token
  on tryout_seasons (rankings_share_token)
  where rankings_share_token is not null;

-- Allow unauthenticated reads via the share token
-- (RLS policies on tryout_seasons already exist; we add a public read policy)
create policy "tryout_seasons_public_share_read"
  on tryout_seasons for select
  using (rankings_share_token is not null);

-- Allow public reads on related tables when joined through a valid season share token.
-- We expose this via a security-definer RPC instead of opening up table policies.
    ...
    'players', coalesce((
      select jsonb_agg(
        jsonb_build_object(
          ...
          'age_group', p.age_group, 'jersey_number', p.jersey_number, 'prior_team', p.prior_team
