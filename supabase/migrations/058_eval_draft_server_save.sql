-- ============================================================
-- Migration 058: Server-side draft persistence for team eval forms
-- ============================================================
--
-- Adds tryout_eval_drafts — one row per team token storing the
-- entire form state (scores, comments, overall notes, coach name).
-- Draft is restored when the coach reopens the link on any device.
-- On submit the draft is marked submitted; the per-player rows in
-- tryout_coach_evals are written as before.
-- ============================================================
...
-- Access is exclusively through security definer RPCs — no direct RLS policy needed.

-- ── Updated save-draft function ───────────────────────────────────────────────
-- Upserts into tryout_eval_drafts (single row per token).
-- Will not overwrite a submitted draft.
...
-- ── Updated form-data function (includes server draft) ────────────────────────
...
-- ── Updated submit function (marks draft as submitted) ────────────────────────
